import { Component, ElementRef, OnInit, PipeTransform, Pipe, ViewChild } from '@angular/core';
...
export class DashProductComponent implements OnInit{
  ...
  async exportarProductos() {
    try {
      const productos = await this.requestsService.get('Productos');
      this.exportarCSV(productos);
    } catch (error) {
